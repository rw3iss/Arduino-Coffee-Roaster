#line 1 "/Users/rw3iss/Documents/Arduino/Projects/Roaster/src/lib/app/components/Component.h"
#pragma once
...
Component::Component(string newType) {
    this->type = newType;
}
...
Component* Component::removeChild(uint16_t index) {
    ...
    if (index > size) throw "removeChild() index is greater than children size.";
...
Component* Component::getChild(uint16_t index) {
    uint16_t size = this->children->size();
    if (index < 0 || index > size) return NULL;  // throw "getChild() index is greater than children size.";
